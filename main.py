...
parser.add_argument('--epochs', default=150, type=int)  # 迭代次数
...
parser.add_argument('--loss', default='RMSE', type=str)  # 损失函数：RMSE / L1 / MSE
...
if __name__ == '__main__':
    ...
    # 设置log路径名
    log_path = ('logs_2026_2_26_{0}_{1}_fusion/{2}_lr{3}_6P'
    ...
    for epoch in range(epochs):
        ...
        writer.add_scalar('train_r2', my_model.average_r2, epoch)
        # 评估模型
        my_model.eval()
        # 记录测试损失
        writer.add_scalar('test_loss', my_model.test_loss, epoch)
        writer.add_scalar('test_loss_inverse', my_model.test_loss_inverse, epoch)
        writer.add_scalar('test_r2', my_model.test_r2, epoch)
        writer.add_scalar('test_r2_inverse', my_model.test_r2_inverse, epoch)
        logger.info(f'Epoch {epoch + 1}, '
                    f'Train Loss: {my_model.average_loss}, '
                    f'Test Loss: {my_model.test_loss}, '
                    f'Inverse Test Loss: {my_model.test_loss_inverse}, '
                    f'Train R2: {my_model.average_r2}, '
                    f'Test R2: {my_model.test_r2}, '
                    f'Inverse Test R2: {my_model.test_r2_inverse}')
        ...
        if my_model.test_loss <= best_test_loss:
            ...
            best_test_r2 = my_model.test_r2
            # 保存最佳模型参数
            save_dir = os.path.join('log/txt_log', log_path)
            os.makedirs(save_dir, exist_ok=True)  # 确保目录存在
            # 动态命名：包含模型名和数据集名，方便区分双路径或不同结构的实验结果
            save_path = os.path.join(save_dir, f'best_{model_name}_{data.name}_weights.pth')
            # 保存前最好将模型参数移至CPU，这样即使在没有GPU的机器上也能轻松加载
            # 注意：此处使用 deepcopy 或者直接保存 t，保存 state_state_dicdict 是最高效的做法
            torch.save(my_model.model.cpu().state_dict(), save_path)

            # 如果你的训练还在使用GPU，保存完后需要把模型重新移回GPU继续训练
            if torch.cuda.is_available():
                my_model.model.cuda()

            logger.info(f'---> Saved new best model weights to {save_path}')

        if (epoch + 1) % 10 == 0:
            logger.info(f'Best Epoch {best_epoch + 1}, '
                        f'Best Test Loss: {best_test_loss:}, '
                        f'Inv Test Loss: {best_inverse_test_loss:}, '
                        f'Best Test R2: {best_test_r2:}')
        writer.close()
