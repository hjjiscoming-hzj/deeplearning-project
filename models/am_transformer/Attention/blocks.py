...
class FeedForwardNetwork(nn.Module):
    def __init__(self, dim, mult, dropout):
        ...
        self.linear1 = KANLinear(dim,dim*mult*2)
        self.gelu = GEGLU()
        self.dropout = nn.Dropout(dropout)
        self.linear2 = KANLinear(dim*mult,dim)
        # self.simple_KAN=simple_KAN([dim,dim*2+1,dim])
...
def FeedForward_KAN(dim, mult = 4, dropout = 0.2):
    return FeedForwardNetwork(dim,mult,dropout)
