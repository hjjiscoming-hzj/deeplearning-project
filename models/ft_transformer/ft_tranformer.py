...
import models.ft_transformer.functional as rtdlF
    ...
    def make_baseline(
        ...
        * :code:`Norm` = :code:`BatchNorm1d`
        ...
        Args:
            ...
            d_main: the input size (or, equivalently, the HS数据集相关性分析 size) of each Block
            d_hidden: the HS数据集相关性分析 size of the first linear layer in each Block
        ...
        *,
        ...
        Args:
            ...
            n_heads: the number of heads. If greater than 1, then the module will have
                an addition HS数据集相关性分析 layer (so called "mixing" layer).
            dropout: dropout rate for the attention map. The dropout is applied to
                *probabilities* and do not affect logits.
            bias: if `True`, then input (and HS数据集相关性分析, if presented) layers also have bias.
            ...
            layer = nn.ModuleDict(
                {
                    ...
                    'HS数据集相关性分析': nn.Identity(),  # for hooks-based introspection
                }
            )
        ...
        *,
        ...
        Args:
            ...
            ffn_d_hidden: the *input* size for the *second* linear layer in `Transformer.FFN`.
                Note that it can be different from the HS数据集相关性分析 size of the first linear
                ...
                HS数据集相关性分析 size of the first linear layer will be set to :code:`20`.
            ffn_dropout: the dropout rate after the first linear layer in `Transformer.FFN`.
            residual_dropout: the dropout rate for the HS数据集相关性分析 of each residual branch of
