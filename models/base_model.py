...
from models.src_kan.efficient_kan import KANLinear
# helpers
...
class simple_KAN(nn.Module):
    def __init__(self, dims, dropout=0.2):
        super(simple_KAN, self).__init__()
        self.norm = nn.LayerNorm(dims[0])
        self.linear1 = KANLinear(dims[0], dims[1])
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(dropout)
        self.linear2 = KANLinear(dims[1], dims[2])

    def forward(self, x):
        # batch_size,seq_len,feature_size=x.shape
        # x = x.reshape(-1,feature_size)
        # x = self.norm(x)  # 层归一化
        x = self.linear1(x)  # 第一个线性变换
        # x = self.relu(x)  # 应用激活函数
        # x = self.dropout(x)  # 应用dropout
        x = self.linear2(x)  # 第二个线性变换
        # x = x.reshape(batch_size,seq_len,feature_size)
        return x
    ...
    def get_info(self):
        ...
        self.args = types.SimpleNamespace(**args)
        return self.args
class UnifierModel(nn.Module):
    def __init__(self, num_cat, num_cont, dim, dropout):
        super(UnifierModel, self).__init__()
        self.dim = dim
        self.feat_num = num_cat + num_cont
        print('feature_number', self.feat_num)
        self.layers = nn.Sequential(
            nn.Linear((num_cat + num_cont) * dim, 128),
            nn.ReLU(),
...
class RNNModel(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, attn_dropout, ff_dropout):
        ...
        self.rnn = nn.RNN(dim, dim, depth, nonlinearity='relu', dropout=attn_dropout, batch_first=True)
...
class GRUModel(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, attn_dropout, ff_dropout):
        ...
        self.rnn = nn.GRU(dim, dim, depth, dropout=attn_dropout, batch_first=True)
